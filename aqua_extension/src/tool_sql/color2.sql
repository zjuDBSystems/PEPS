WITH
  ...
  Conv170_fwd0 AS (
	SELECT id, kfm_nt(W.val, I.value) AS value
  ...
  Conv177_fwd0 AS (
	...
	kfm_im2col(W.val, I.value, kernel:=3, padding:=1, stride:=1)) AS value
  ...
  Conv215_fwd0 AS (
	SELECT id, kfm_im2col(W.val, I.value, kernel:=1, padding:=0, stride:=2) AS value
  ...
  Conv219_fwd0 AS (
	SELECT id, array_2d_agg(
	kfm_im2col(W.val, I.value, kernel:=3, padding:=1, stride:=2)) AS value
  ...
  AveragePool297_fwd0 AS (
	SELECT id, avgpool(I.value, kernel:=8, stride:=8) AS value
	FROM Relu295_fwd0 I
	),
  Reshape298_fwd0 AS (
