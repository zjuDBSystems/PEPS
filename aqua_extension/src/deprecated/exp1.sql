WITH
  ...
  AveragePool293_fwd0 AS (
	SELECT id, avgpool(I.value, kernel:=3, padding:=1, stride:=1) AS value
	FROM Conv506_fwd0 I
	),
  Conv512_fwd0 AS (
	SELECT id, kfm_nt(W.val, I.value, W.bias) AS value
	FROM AveragePool293_fwd0 I, Conv512_weight W
	),	 
  Conv507_fwd0 AS (
	SELECT id, kfm_nt(W.val, I.value, W.bias) AS value
  ...
  Conv519_fwd0 AS (
	SELECT id, kfm_nt(W.val, I.value, W.bias) AS value
	FROM AveragePool310_fwd0 I, Conv519_weight W
	),
  Conv517_fwd0 AS (
	SELECT id, kfm_im2col(W.val, I.value, W.bias, kernel:=5, padding:=2, stride:=1) AS value
  ...
  Conv526_fwd0 AS (
	SELECT id, kfm_nt(W.val, I.value, W.bias) AS value
	FROM AveragePool327_fwd0 I, Conv526_weight W
	),
  Conv524_fwd0 AS (
	SELECT id, kfm_im2col(W.val, I.value, W.bias, kernel:=5, padding:=2, stride:=1) AS value
  ...
  Conv535_fwd0 AS (
	SELECT id, kfm_nt(W.val, I.value, W.bias) AS value
	FROM AveragePool355_fwd0 I, Conv535_weight W
	),
  Conv533_fwd0 AS (
  ...
  Conv545_fwd0 AS (
	SELECT id, kfm_nt(W.val, I.value, W.bias) AS value
	FROM AveragePool378_fwd0 I, Conv545_weight W
	),
  Conv543_fwd0 AS (
	SELECT id, kfm_im2col_ns(
		W.val, I.value, W.bias, 1, 7, 0, 3, 1
  ...
  Conv555_fwd0 AS (
	SELECT id, kfm_nt(W.val, I.value, W.bias) AS value
	FROM AveragePool401_fwd0 I, Conv555_weight W
	),
  Conv553_fwd0 AS (
	SELECT id, kfm_im2col_ns(
		W.val, I.value, W.bias, 1, 7, 0, 3, 1
  ...
  Conv565_fwd0 AS (
	SELECT id, kfm_nt(W.val, I.value, W.bias) AS value
	FROM AveragePool424_fwd0 I, Conv565_weight W
	),
  Conv563_fwd0 AS (
	SELECT id, kfm_im2col_ns(
		W.val, I.value, W.bias, 1, 7, 0, 3, 1
  ...
  Conv580_fwd0 AS (
	SELECT id, kfm_nt(W.val, I.value, W.bias) AS value
	FROM AveragePool462_fwd0 I, Conv580_weight W
	),
  Conv579_fwd0 AS (
	SELECT id, kfm_im2col(W.val, I.value, W.bias, kernel:=3, padding:=1, stride:=1) AS value
  ...
  Conv589_fwd0 AS (
	SELECT id, kfm_nt(W.val, I.value, W.bias) AS value
	FROM AveragePool485_fwd0 I, Conv589_weight W
	),
  Concat494_fwd0 AS (
	SELECT id, concat_array(2,
	input1:=kfm_im2col_ns(
  ...
  MatMul505_fwd0 AS (
	SELECT id, mvm(W.val, I.value, bias:=W.bias) AS value
